...
export const getTicketById = async (
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_support_ticket_by_id", {
      p_user_id: userId,
      p_ticket_id: id,
    });
...
export const createMessage = async (
  ...
  try {
    ...
    // Se um admin respondeu, enviar email para o dono do ticket
    if (userIsAdmin) {
      try {
        ...
        if (ticketData) {
          ...
          if (userProfile && userProfile.email) {
            ...
            console.log(
              `📧 Email de notificação enviado para ${userProfile.email}`,
            );
          }
        }
      } catch (emailError) {
        // Apenas logar o erro, não falhar a requisição
